...
def list_sops(db: DB, _user: CurrentUser):
    return db.table("sop_entries").select("*").order("updated_at", desc=True).execute().data


@router.get("/{slug}", response_model=SopOut)
def get_sop(slug: str, db: DB, _user: CurrentUser):
    result = db.table("sop_entries").select("*").eq("slug", slug).limit(1).execute()
    if not result.data:
...
def create_sop(body: SopCreate, db: DB, user: EditorUser):
    existing = db.table("sop_entries").select("id").eq("slug", body.slug).limit(1).execute()
    if existing.data:
        raise HTTPException(status_code=409, detail="Slug already exists")
    result = db.table("sop_entries").insert({
        "slug": body.slug,
        "title": body.title,
...
def update_sop(slug: str, body: SopUpdate, db: DB, user: EditorUser):
    existing = db.table("sop_entries").select("id").eq("slug", slug).limit(1).execute()
    ...
    result = db.table("sop_entries").update(updates).eq("slug", slug).execute()
    return result.data[0] if result.data else {}
...
def delete_sop(slug: str, db: DB, _user: EditorUser):
    ...
    if not existing.data:
        raise HTTPException(status_code=404, detail="SOP not found")
    db.table("sop_entries").delete().eq("slug", slug).execute()
